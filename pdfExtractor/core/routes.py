...
class PDFUpload(Resource):
    def post(self) -> dict:
        ...
        create_task = {"status": result.status, "file_name": uploaded_file.filename, "upload_directory": upload_folder}
        update_task_model(result.id, **create_task)

        return {'message': 'File uploaded successfully', 'filename': uploaded_file.filename, "result_id": result.id}
